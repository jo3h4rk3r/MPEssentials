...
public class MsgIgnoreCommand {
    private static int count = 0;
    public static HashMap<UUID, UUID> MsgIgnoreC = new HashMap<UUID, UUID>();
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, boolean b) {
        dispatcher.register(CommandManager.literal("tpaignore").then(CommandManager.argument("player", EntityArgumentType.player()).executes((context) -> {
            ...
            if (count == 0) {
                ...
                MsgIgnoreC.put(targetedPlayer.getUuid(), playerEntity.getUuid());
                count++;
            } else {
                MsgIgnoreC.remove(targetedPlayer.getUuid(), playerEntity.getUuid());
